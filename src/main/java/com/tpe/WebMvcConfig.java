...
public class WebMvcConfig implements WebMvcConfigurer {
    ...
    //to add ViewResolver obj as bean
    @Bean
    public InternalResourceViewResolver resolver(){
        InternalResourceViewResolver resolver = new InternalResourceViewResolver();
        resolver.setViewClass(JstlView.class); //JavaStandartLibrary --> to writel less codes inside jsp
        resolver.setPrefix("/WEB-INF/views"); //indicating location of view files
        resolver.setSuffix(".jps"); // extentions of view files
        return resolver();
    }
    ...
    public void addResourceHandlers(ResourceHandlerRegistry registry) {
        registry.addResourceHandler("/resources/**"). //the path for static files
                addResourceLocations("/resources/"). // this is to indicate location
